...
- (Optional) Draws compact trade markers (entry/stop/TP1/TP2) whenever a block is retested, and greys them out when the block is filled or expires; stops sit 6 ticks beyond the zone edge and targets lock to the nearest opposing imbalance (no fallback target is plotted if none exists).
...
- **Bars to Keep Unfilled Blocks** / **Key Level Lifespan** / **Max Stored Key Levels** / **Bars to Keep Inactive Blocks**: Control how long active, historical, and inactive (filled/expired) imbalances remain visible before fading or deleting.
- **Trade Markers**: Toggle plotting trade cues, set entry offsets, and configure TP behavior (defaults to nearest opposing level with a fixed 6-tick stop beyond the zone; if no opposing level exists, no TP markers are drawn).
